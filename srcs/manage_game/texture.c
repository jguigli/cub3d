...
{
    main->texture[0].addr = (int *)mlx_get_data_addr(main->texture[0].mlx_img, &main->texture[0].bpp, &main->texture[0].line_len, &main->texture[0].endian);
	if (!main->texture[0].addr)
		return (1);
    main->texture[1].addr = (int *)mlx_get_data_addr(main->texture[1].mlx_img, &main->texture[1].bpp, &main->texture[1].line_len, &main->texture[1].endian);
	if (!main->texture[1].addr)
		return (1);
    main->texture[2].addr = (int *)mlx_get_data_addr(main->texture[2].mlx_img, &main->texture[2].bpp, &main->texture[2].line_len, &main->texture[2].endian);
	if (!main->texture[2].addr)
		return (1);
    main->texture[3].addr = (int *)mlx_get_data_addr(main->texture[3].mlx_img, &main->texture[3].bpp, &main->texture[3].line_len, &main->texture[3].endian);
...
void	display_color_texture(t_main *main, int x)
{
	...
	while (y <= main->ray->drawend)
	{
		...
		if (y < main->scr_y && x < main->scr_x)
			main->img->addr[y * main->img->line_len / 4 + x] = main->texture[main->tex->texdir].addr[main->tex->texY * main->texture[main->tex->texdir].height + main->tex->texX];
		y++;
	}
